...
if recent_papers:
    for paper in recent_papers:
        with st.expander(
            f"{paper.get('subject', 'N/A')} - {paper.get('chapter', 'N/A')} | Grade: {paper.get('grade', 'N/A')} | {utils.format_date(paper['created_at'])}"
        ):
            col1, col2, col3 = st.columns(3)
            with col1:
                st.markdown(f"**Paper ID:** {paper['id']}")
                st.markdown(f"**Type:** {paper.get('type', 'N/A')}")
            with col2:
            ...
            with col3:
                ...
                if paper.get('document_name'):
                    st.markdown(f"**Document:** {paper['document_name']}")
else:
...
with col2:
    if st.button("✅ Evaluate Paper", use_container_width=True, type="primary"):
